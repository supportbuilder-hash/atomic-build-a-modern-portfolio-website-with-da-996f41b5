...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <section id="home" className="relative min-h-[92vh] flex items-center overflow-hidden">
        ...
        <div className="w-full max-w-6xl mx-auto px-6 md:px-10 py-28 md:py-36">
          <motion.div variants={heroContainer} initial="hidden" animate="visible" className="max-w-3xl">
            ...
            {/* Headline */}
            <motion.h1 variants={heroLine} className="text-5xl sm:text-6xl md:text-7xl font-bold tracking-tight leading-[1.05] text-balance mb-6">
              {t("hero.greeting")}{" "}
            ...
            {/* Subhead */}
            <motion.p variants={heroLine} className="text-lg md:text-xl text-[var(--foreground)]/60 leading-relaxed max-w-xl mb-10 text-pretty">
              {t("hero.tagline")}
            </motion.p>

            {/* CTAs */}
            <motion.div variants={heroLine} className="flex flex-wrap items-center gap-4 mb-14">
              <Link href="/projects" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[var(--brand-primary)] text-[var(--brand-primary-fg)] font-semibold text-sm hover:opacity-90 transition-all duration-300 shadow-[0_0_24px_rgba(var(--brand-primary-rgb),0.35)] hover:shadow-[0_0_36px_rgba(var(--brand-primary-rgb),0.5)] hover:-translate-y-0.5">My work</Link>
              <Link href="/contact" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-[var(--foreground)]/15 bg-[var(--foreground)]/5 text-[var(--foreground)] font-semibold text-sm hover:bg-[var(--foreground)]/10 hover:border-[var(--foreground)]/25 transition-all duration-300 hover:-translate-y-0.5">
                {t("hero.cta_secondary")}
              </Link>
            </motion.div>

            {/* Social links */}
            <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex items-center gap-5">
              {socialLinks.map((s) => {
                const Icon = SOCIAL_ICONS[s.platform] ?? Github;
                return (
                  <motion.a key={s.key} href={s.url} target="_blank" rel="noopener noreferrer" aria-label={s.platform} variants={socialIconVariant} whileHover={{ scale: 1.15, y: -2 }} whileTap={{ scale: 0.95 }} className="text-[var(--foreground)]/40 hover:text-[var(--brand-primary)] transition-colors duration-200">
        ...
        {/* Scroll indicator */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4, duration: 0.6 }} className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5" aria-hidden="true">
          <span className="text-[10px] uppercase tracking-widest text-[var(--foreground)]/30">
            {t("hero.scroll")}
          </span>
          <motion.div animate={{ y: [0, 6, 0] }} transition={{ repeat: Infinity, duration: 1.6, ease: "easeInOut" }} className="w-px h-8 bg-gradient-to-b from-[var(--foreground)]/30 to-transparent" />
        </motion.div>
      </section>
      {/* ── Stats bar ────────────────────────────────────────────────────────── */}
      <Reveal>
        <section className="border-y border-[var(--foreground)]/8 bg-[var(--foreground)]/[0.02]">
          <div className="max-w-6xl mx-auto px-6 md:px-10 py-12">
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-0 md:divide-x divide-[var(--foreground)]/10">
              {STATS.map((stat, i) => (
                <motion.div key={i} variants={fadeInUp} className="flex flex-col items-center text-center md:px-8">
          ...
          <div className="max-w-6xl mx-auto px-6 md:px-10">
            ...
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {SERVICES.map((svc, i) => {
                const Icon = svc.icon;
                return (
                  <motion.div key={i} variants={scaleIn} whileHover={{ y: -4 }} className="group relative p-7 rounded-2xl border border-[var(--foreground)]/8 bg-[var(--foreground)]/[0.03] hover:border-[var(--brand-primary)]/30 hover:bg-[var(--brand-primary)]/5 transition-all duration-300 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-8px_rgba(0,0,0,0.12)]">
      ...
      {/* ── Featured Projects ─────────────────────────────────────────────────── */}
      <Reveal>
        <section id="projects" className="py-24 md:py-32 bg-[var(--foreground)]/[0.025] border-y border-[var(--foreground)]/8">
          <div className="max-w-6xl mx-auto px-6 md:px-10">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-16">
              ...
              <Link href="/projects" className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--brand-primary)] hover:gap-2.5 transition-all duration-200 shrink-0">
                {t("projects.view_all")}
            ...
            <div className="flex flex-col gap-8">
              {FEATURED_PROJECTS.map((project, i) => (
                <Reveal key={project.id} delay={i * 0.08}>
                  <motion.article whileHover={{ y: -3 }} className="group grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-0 rounded-2xl border border-[var(--foreground)]/8 bg-[var(--background)] overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-8px_rgba(0,0,0,0.14)] hover:border-[var(--brand-primary)]/25 transition-all duration-300">
                    {/* Image */}
                    ...
                    <div className="p-7 md:p-9 flex flex-col justify-between">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {project.tech.map((t) => (<span key={t} className="text-xs px-2.5 py-1 rounded-md border border-[var(--foreground)]/10 bg-[var(--foreground)]/5 text-[var(--foreground)]/60">
                            {t}
                          </span>))}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4 mt-6 pt-6 border-t border-[var(--foreground)]/8">
                        {project.githubUrl && (
                          <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-medium text-[var(--foreground)]/50 hover:text-[var(--foreground)] transition-colors duration-200">
                            <Github className="h-3.5 w-3.5" />
                            {t("projects.source")}
                          </a>
                        )}
                        {project.liveUrl && (
                          <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-medium text-[var(--brand-primary)] hover:opacity-80 transition-opacity duration-200">
          ...
          <div className="max-w-6xl mx-auto px-6 md:px-10">
            ...
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {TESTIMONIALS.map((item, i) => (
                <motion.figure key={i} variants={fadeInUp} whileHover={{ y: -4 }} className="relative flex flex-col p-7 rounded-2xl border border-[var(--foreground)]/8 bg-[var(--foreground)]/[0.03] shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-8px_rgba(0,0,0,0.12)] hover:border-[var(--brand-primary)]/25 transition-all duration-300">
      ...
      {/* ── CTA ──────────────────────────────────────────────────────────────── */}
      <Reveal>
        <section id="contact" className="py-24 md:py-32 border-t border-[var(--foreground)]/8">
          <div className="max-w-6xl mx-auto px-6 md:px-10">
            <div className="relative rounded-3xl overflow-hidden border border-[var(--brand-primary)]/20 bg-[var(--brand-primary)]/8 px-8 md:px-16 py-16 md:py-20 text-center">
              ...
              <div className="flex flex-wrap items-center justify-center gap-4">
                <Link href="/contact" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl bg-[var(--brand-primary)] text-[var(--brand-primary-fg)] font-semibold text-sm hover:opacity-90 transition-all duration-300 shadow-[0_0_28px_rgba(var(--brand-primary-rgb),0.4)] hover:shadow-[0_0_40px_rgba(var(--brand-primary-rgb),0.55)] hover:-translate-y-0.5">
                  {t("cta.button_primary")}
                  <ArrowRight className="h-4 w-4" />
                </Link>
                <a href={BRAND.resumeUrl} download className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl border border-[var(--foreground)]/15 bg-[var(--foreground)]/5 text-[var(--foreground)] font-semibold text-sm hover:bg-[var(--foreground)]/10 transition-all duration-300 hover:-translate-y-0.5">
              ...
              <p className="mt-8 text-xs text-[var(--foreground)]/35">
                {t("cta.footnote")}{" "}
                <a href={`mailto:${BRAND.email}`} className="text-[var(--brand-primary)] hover:underline">
                  {BRAND.email}
                </a>
              </p>
            </div>
